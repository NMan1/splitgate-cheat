...
namespace ZeroGUI
{
	...
	void TextLeft(const char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		...
		canvas->K2_DrawText(render::esp_font, FString{ s2wc(name) }, pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, false, true, true, Colors::Text_Outline);
	}

	void TextCenter(const char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		int length = strlen(name) + 1;
		canvas->K2_DrawText(render::esp_font, FString{ s2wc(name) }, pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, true, true, true, Colors::Text_Outline);
	}
